...
| File                                                                                                         | Role                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CreatureData.cs`                                                                                            | ScriptableObject. Per-species config: visuals per rarity, movement/detection/flee tuning, resources per rarity, `family` (`IngredientFamily`), `favoriteScent`/`hatedScent` (each a single `ScentType`, dropdown — not a weighted profile). `GetScentAffinity()` scores both against the active stew's scent profile — used by `Spawner` (weight) and `CreatureAI` (detection radius). See decision log |
| `CreatureAI.cs`                                                                                              | State machine (Idle/Wander/Flee/Stunned/Captured). Rolls rarity ONCE per instance in `Awake`, never writes back to the shared `CreatureData`. Implements `ICapturable`. Rarity roll and flee behavior are live-modified by `ExpeditionStewManager` (Shiny Power / Soothing Power, both family-scoped). Has hit-point health now (no visual bar) — `OnHit(damage)` only stuns at 0 HP, see `CaptureMinigameConfig`. A successful capture also rolls Ingredient Power + the tool's own extra-ingredient chance (`AddCreature`'s `doubleYield` flag) and applies Chrono Power's per-capture time bonus — see decision log |
...
| `CaptureMinigameConfig.cs`     | ScriptableObject. Rarity-indexed (0-3) barrier count/width/movement and creature max health — read via `CaptureMinigameController.Instance`, not held directly by `CreatureAI` (scene-load-order reasons) — see decision log       |
